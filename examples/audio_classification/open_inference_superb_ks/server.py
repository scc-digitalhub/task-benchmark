...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if unquote(self.path) == f"/v2/models/{MODEL}":
            self.send_json({
                "name": MODEL,
                "inputs": [{"name": "audio", "datatype": "BYTES", "shape": [-1]}],
    ...
    def do_POST(self):
        ...
        try:
            ...
            data = next(item["data"] for item in body["inputs"] if item["name"] == "audio")
            sample_rate = body["parameters"]["sample_rate"]
            audio = [
                {"array": np.frombuffer(base64.b64decode(item), dtype=np.float32), "sampling_rate": sample_rate}
                for item in data
            ]
